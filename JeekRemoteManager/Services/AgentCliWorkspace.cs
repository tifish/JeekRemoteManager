...
public static class AgentCliWorkspace
{
        ...
        string? mcpEndpointUrl)
    {
        ...
        sb.AppendLine("session. Prefer **one tool call per step**, wait for the result, then decide the");
        sb.AppendLine("next step. When finished, reply with a short summary and no further tool calls.");
        sb.AppendLine();
