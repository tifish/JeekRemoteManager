...
public static class AgentCliWorkspace
{
        ...
        AgentEndpointSettings? codexEndpoint = null)
    {
        ...
        WriteProjectMcpConfigs(absolute, connectionPath, mcpToolsAutoApprove, codexEndpoint);
        return absolute;
    }
        ...
        int sessionNumber)
    {
        ...
        // Full context lives in AGENTS.md (Codex/Grok/Copilot/Cursor/shared). Agents with their
        // own context file name get a one-line include so we never maintain two copies.
        ...
        {
            File.WriteAllText(
                Path.Combine(workspaceDir, include),
        ...
        AgentEndpointSettings? codexEndpoint = null)
    {
        ...
        // Gemini CLI stopped serving individual accounts in June 2026 and was replaced by
        // Antigravity, which reads .agents/mcp_config.json and AGENTS.md. Drop what we used to
        // write for it so a stale config cannot point an agent at a dead CLI.
        TryDelete(Path.Combine(workspaceDir, "GEMINI.md"));
        TryDeleteFolder(Path.Combine(workspaceDir, ".gemini"));
        ...
        foreach (var target in AgentMcpConfigCatalog.All)
        {
            ...
            var content = target.Format == AgentMcpConfigCatalog.ConfigFormat.Json
                ...
                : "# Generated by JeekRemoteManager — per-connection remote tools\n"
                  ...
                  // Codex takes its endpoint from config, not the command line. The key is not
                  // here — the block only names the environment variable carrying it.
                  + (target.SupportsApprovalMode
                      ? AgentEndpointConfig.BuildCodexProviderToml(codexEndpoint)
                      : "")
    ...
    /// <summary>
    /// Removes a config folder this generator no longer writes. Safe because everything under
    /// the workspace is ours — unlike a linked project, where we only ever take our own entries out.
    /// </summary>
