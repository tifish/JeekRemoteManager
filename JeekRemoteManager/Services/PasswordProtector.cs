...
/// user's master password (see <see cref="MasterKeyService"/>): each password is
/// stored as a self-contained jrm1 envelope with its own random salt and AES-GCM
/// nonce. This keeps a single connection file portable across machines once the
/// master password is re-entered.
///
...
{
    /// <summary>Encrypts a clear-text password with the session master password; returns a jrm1 blob.</summary>
    public static string Encrypt(string? clearText) =>
        MasterKeyService.Current?.EncryptPassword(clearText) ?? "";

    /// <summary>Decrypts a jrm1 blob produced by <see cref="Encrypt"/>. Returns "" on failure.</summary>
    ...
    /// decrypted with the current master password. Use the false result to preserve the
