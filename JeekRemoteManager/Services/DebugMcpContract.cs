...
public static class DebugMcpContract
{
    ...
    public static JsonArray BuildToolList() => new(
        ...
        Tool("settings_dialog_layout_check",
            ...
            new()),
        Tool("settings_terminal_preview_check",
            "Open the real Settings dialog, verify the terminal font list holds only monospace families, that changing the font and color scheme applies to open terminals immediately, and that Cancel restores the saved appearance.",
            new()),
        ...
        Tool("connection_write_watcher_check",
            ...
            new()),
        Tool("connection_external_change_check",
            "Makes an own connection write, then writes a connection file from outside the store 200 ms later, and verifies the tree shows the external file; also verifies an own write alone reloads the tree exactly once because the watcher recognises it.",
            new()),
        Tool("connection_tree_read_cache_check",
            "Builds a throwaway connection tree and verifies a repeat ReadTree reads no unchanged files, a file changed on disk is read again with its new content, and each read hands out fresh Connection objects.",
            new()),
        ...
        Tool("host_key_trust_check",
            "Verifies first-seen SSH host keys are saved without prompting, while changed remembered keys require replacement confirmation. Also checks the remembered key family is recorded and offered first, legacy entries learn their family, and a corrupt known_hosts file is backed up rather than overwritten.",
            new()),
        Tool("sftp_host_key_check",
            "Dials a real SFTP session (default: the local WSL sshd rig at jrmtest@127.0.0.1:2222) and verifies a planted mismatched host key is rejected, a forgotten host is trusted on first use, and a client ranking another key family first still negotiates the remembered one. Restores the host's original known-hosts entry.",
        ...
        Tool("ssh_jump_forward_check",
            "Against a real sshd (default: the local WSL rig jrmtest@127.0.0.1:2222): dials through a jump host and runs a command, pushes bytes through L, D (SOCKS5) and R forwards, checks forwards stop when their transport is released, and checks parse errors and an unknown jump host are reported. Restores the rig's known-hosts entry.",
            new() { ["host"] = Prop("string", "Test server host (default 127.0.0.1)."), ["port"] = Prop("integer", "Test server port (default 2222)."), ["username"] = Prop("string", "User with key auth (default jrmtest).") }),
        Tool("vnc_launch_check",
            "Checks the VNC launch path without a VNC server: reports the TigerVNC viewer and winget found, verifies the winget install command, the viewer's server-argument format (display vs port, IPv6), that the password travels only in the child's VNC_PASSWORD environment (never the command line, never inherited), and, against a real sshd (default: the local WSL rig jrmtest@127.0.0.1:2222), that a tunneled launch points a stand-in viewer at a local forward that reaches the target and closes when the viewer exits. Restores the rig's known-hosts entry.",
            new() { ["host"] = Prop("string", "Test server host (default 127.0.0.1)."), ["port"] = Prop("integer", "Test server port (default 2222)."), ["username"] = Prop("string", "User with key auth (default jrmtest).") }),
        ...
        Tool("zmodem_subpacket_limit_check",
            ...
            new()),
        Tool("terminal_appearance_check",
            "Applies a terminal font, color scheme and scrollback, opens a tab and verifies it got the font and scrollback, then renders it and checks background and cached text colors follow a scheme switch. Restores the original appearance.",
            new()),
        Tool("terminal_find_check",
            "Fills a probe terminal tab with 300 lines holding three hits (one deep in scrollback, one uppercase), searches through the real find bar, steps next/previous, adds output and steps again, then checks a no-hit search and closing the bar.",
            new()),
        Tool("terminal_session_log_check",
            "Records a probe tab to a temporary session log while feeding colors, an OSC title, an escape split across packets, CRLF, a progress-bar carriage return and Chinese, then verifies the file holds exactly the readable text with start/end markers.",
            new()),
        Tool("terminal_encoding_check",
            "Verifies a GBK terminal encoding applies at every text boundary: output split mid-character renders intact in a real terminal tab, typed input is re-encoded as GBK, captured script output and login-menu capture decode GBK.",
            new()),
        ...
        Tool("product_mcp_check",
            ...
            new()),
        Tool("mcp_adapter_offline_check",
            "Runs the stdio adapter beside this build against a pipe nobody serves and verifies it advertises tools.listChanged and lists the full product contract offline; then serves that pipe with a fake app offering a different tool list and verifies the next call is followed by notifications/tools/list_changed.",
            new()),
