...
设置窗口按“页头说明 / 可滚动内容 / 固定操作栏”分成三段。内容按外观、文件与存储、安全、更新四张卡片组织；窗口允许缩放，小尺寸下只滚动中间内容，确定和取消按钮始终可见。底部按钮遵循 Windows 从左到右“确定、取消”的顺序，并整体靠右。`settings_dialog_layout_check` 打开真实设置窗口并验证这个结构与按钮顺序，防止新增设置项重新退化成无分组的长列表。
