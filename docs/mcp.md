...
2. **任何需要用户的事情都发生在 GUI 里。** 用户必须输入的秘密（主密码、二次因子）在 GUI 输入，绝不作为工具参数；破坏性动作在 GUI 确认。远程 shell 命令不做内容检查或执行确认，统一通过 `terminal_run`/`terminal_run_batch` 直接执行。另一个例外是 `connection_delete`/`folder_delete`：删除树节点一律进回收站、可恢复，所以不弹确认，免得 agent 卡在对话框上。工具的做法是激活窗口并返回一个 agent 可以轮询的状态（如 `awaiting_user`），而不是无限期地阻塞一次工具调用。
