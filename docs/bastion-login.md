...
- 每个条目记录"最后已知的逻辑目标"（`BastionRoute`）。这一点很重要：**新通道落在哪里由堡垒机决定，不能想当然，而且同一台堡垒机上都不是固定的**。齐治交互终端实测两种都会出现：上一个资产的会话还开着时，新通道被自动接回那个资产，直接落进目标 shell、根本不出菜单；那条会话已经退出时，新通道停在资产菜单上。所以路由只是"记得的"，真正算数的是打开通道后看见的落点。
...
主窗口隐藏到托盘不结束池的生命周期：只有实际 `Closed` 才释放整个池，不能在会被取消的 `Closing` 中释放。否则所有后续登录都会成为 `fresh-pool-rejected`，即使原终端已经过二次验证也无法复用。Debug MCP 的 `bastion_tray_lifecycle_check` 覆盖此边界。
