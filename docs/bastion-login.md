...
- 新传输在 **2FA 一过就入池**（这样后面的目标不用再验一次），但那时它还在堡垒机入口，所以登记的是 `AtEntry` 而不是目标；等登录序列真的到达目标，路由才被改写成目标。登录中途失败则登记 `Unknown`。**登记一个还没到达的路由，会让下一个借用者把那个目标的 `#reuse-leave` 发进当前屏幕。**
- 认证本身也要**互斥**：拨号前用 `TryReserveFreshLogin` 占住这个身份，其他连接看到有人在认证就 `WaitForFreshLoginAsync` 等着，等到传输入池再借。否则"连着双击两个连接"必然各拨各的，用户被要两次验证码。占位在传输入池那一刻就释放（等的人不必陪着走完菜单），并且无论成败都在收尾时释放，绝不能把别人永久挂住。
- 池键是**哈希**的，诊断快照里只出现路由名和端点标签，不出现凭据。
- 每个条目记录"最后已知的逻辑目标"（`BastionRoute`）。这一点很重要：**新通道落在哪里由堡垒机决定，不能想当然**。实测的齐治交互终端会把新开的 shell 通道自动接回该传输上一次选中的资产，直接落进旧目标的 shell，根本不出菜单；也有堡垒机把新通道停在菜单上。池里记住旧路由，就是为了让下一个借用者知道要不要先退出来。
...
- `KeepAndRelease()` —— 借用失败或被取消，引用还回去。**保留已认证传输**，只把路由标记为未知。
- `AbandonAndTakeClient()` —— 保留引用给手动接管，但把池条目摘掉，因为传输本身已不可用。
- `Abandon()` —— 摘掉池条目。**只有传输本身不可用时才用它**：借用失败该用 `KeepAndRelease()`，丢掉一条已认证传输等于让下一个连接重过一遍 2FA。

复用的登录命令没走到目标时，终端会**换一条新通道自动重试一次**（新通道的落点是干净的，且不需要重新登录），再失败就打印一句明确的说明、把这条堡垒机会话留给用户手工接管，而**不是**自动去新建连接——那意味着再要一次验证码，而且如果是工作流本身写错了，新建连接同样会失败。
