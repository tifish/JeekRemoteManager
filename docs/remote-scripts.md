...
- 退出结果**不能在 `Append` 里就释放**。调用方拿到完成通知后会直接往终端写 `[script exit N]`，而这一包的显示字节还排在输出帧缓冲里。渲染流的调用方用 `DeferExitCompletion` + `ReleasePendingExit()` 把释放推迟到字节入队之后，细节见 [终端通道与输出管线](terminal-pipeline.md)。
